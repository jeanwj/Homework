...
public class SList extends List {
	 ...
	 * first node. tail references the last node.
	 **/
	 ...
	 * == null and the SListNode referenced by tail can be reached from the head
	 * by a sequence of zero or more "next" references. This implies that the
	 * list is not circularly linked. 2) The "size" field is the number of
	 * SListNodes that can be accessed from head (including head itself) by a
	 * sequence of "next" references. 3) For every SListNode x in an SList l,
	 * x.myList = l.
	 */
	...
	/**
	 * back() returns the node at the back of this SList. If the SList is empty,
	 * return an "invalid" node--a node with the property that any attempt to
	 * use it will cause an exception.
	 * 
