...
public class DList extends List {
	 ...
	 * 6) For every DList l, l.head.myList = null. (Note that l.head is the sentinel.) 
	 * 7) For every DListNode x in a DList l EXCEPT l.head (the sentinel), x.myList = l. 
	 * 8) size is the number of DListNodes, NOT COUNTING the sentinel, 
	 *    that can be accessed from the sentinel (head) by a sequence of "next" references.
	 */
	...
	public DList() {
		...
		this.head = newNode(null, this, null, null);
		this.size = 0;
	}
	...
	public void insertFront(Object item) {
		...
		// the `list' field (second parameter) as well.
		if(this.size == 0){
			this.head.prev = this.head.next = this.newNode(item, this, head, head);
		}else{
			DListNode newNode = this.newNode(item, this, head, head.next);
	...
	public void insertBack(Object item) {
		...
		if(this.size == 0){
			...
			DListNode newNode = this.newNode(item, this, head.prev, head);
