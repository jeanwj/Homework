...
public class RunLengthEncoding implements Iterable {
	...
	private DoubleLinkedList<Run> pixels;
	...
	public RunLengthEncoding(int width, int height) {
		...
		this.width = width;
		this.height = height;
		this.pixels = new DoubleLinkedList<Run>();
		this.pixels.addTail(new Run(0, width * height));
	}
	...
	public RunLengthEncoding(int width, int height, int[] red, int[] green,
		...
		for(int i = 0; i < runLengths.length; i++){
			this.pixels.addTail( new Run(ImageUtils.makeRGB(
					(short)red[i], 
					(short)green[i], 
	...
	public RunIterator iterator() {
		...
		if(this.pixels.length == 0)
			...
			return new RunIterator(this.pixels.head);
	...
	public PixImage toPixImage() {
		...
		DoubleLinkedListNode<Run> node = this.pixels.head;
		while(node != null){
			for(int i = 0; i < node.value.count; i++, index++){
				x = index % width;
				y = index / width;
				red = ImageUtils.getRedValue(node.value.pixel);
				green = ImageUtils.getGreenValue(node.value.pixel);
				blue = ImageUtils.getBlueValue(node.value.pixel);
	...
	public RunLengthEncoding(PixImage image) {
		...
		for(int y = 0; y < this.height; y++){
			for(int x = 0; x < this.width; x++){
				...
				pixel = ImageUtils.makeRGB(red, green, blue);
				if(this.pixels.tail == null || this.pixels.tail.value.pixel != pixel){
					this.pixels.addTail(new Run(pixel, 1));
				} else {
					this.pixels.tail.value.count++;
				}
			}
		}
		check();
	...
	public void setPixel(int x, int y, short red, short green, short blue) {
		...
		int pixel = ImageUtils.makeRGB(red, green, blue);
		
		int location = x + y * width;
		int index = 0;
		DoubleLinkedListNode<Run> node = this.pixels.head;
		while(node != null){
			if(location < node.value.count){
				if(pixel == node.value.pixel)
					return;
				if(node.value.count == 1){
					node.value.pixel = pixel;
					return;
				}
				if(node == this.pixels.head){
					if(pixel == node.next.value.pixel){
						node.next.value.count++;
					}else{
						Run newRun = new Run(pixel, 1);
						this.pixels.addHead(newRun);
					}
					node.value.count--;
				} else if(node == this.pixels.tail){
					if(pixel == node.prev.value.pixel){
						node.prev.value.count++;
					}else{
						Run newRun = new Run(pixel, 1);
						this.pixels.addTail(newRun);
					}
					node.value.count--;
				}
				else
				{
					if(pixel == node.prev.value.pixel){
						node.prev.value.count++;
						node.value.count--;
					}else if(pixel == node.next.value.pixel){
						node.next.value.count++;
						node.value.count--;
					}else{
						this.pixels.removeAt(index);
						int rightSum = node.value.count - index - 1;
						if(rightSum > 0){
							Run right = new Run(node.value.pixel, node.value.count - index - 1);
							this.pixels.insertAt(index, right);
						}
						
						Run newRun = new Run(pixel, 1);
						this.pixels.insertAt(index, newRun);
						
						if(index > 0){
							Run left = new Run(node.value.pixel, index);
							this.pixels.insertAt(index, left);
						}
					}
				}
				break;
			}
			location -= node.value.count;
			node = node.next;
			index++;
		}
		check();
	...
	public static void main(String[] args) {
		...
		System.out
				.println("Testing one-parameter RunLengthEncoding constuctor "
		...
		/*
		 * array2PixImage(new int[][] { { 42, 3, 6 }, { 1, 4, 7 }, { 2, 5, 8 }
		 * })),
		 */
		...
		PixImage image3 = array2PixImage(new int[][] { { 0, 5 }, { 1, 6 },
				{ 2, 7 }, { 3, 8 }, { 4, 9 } });
		...
		PixImage image4 = array2PixImage(new int[][] { { 0, 3 }, { 1, 4 },
				{ 2, 5 } });
...
class Run{
	...
	@Override
	public String toString(){
		return "{\"pixel\":" + pixel + ", \"count\":" + count +  "}";
	}
}
