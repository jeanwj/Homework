...
public class ImageUtils {
	...
	private static PixImage buffer2PixImage(BufferedImage bImage) {
		...
		for (int x = 0; x < bImage.getWidth(); x++) {
			for (int y = 0; y < bImage.getHeight(); y++) {
				...
				pImage.setPixel(x, y, (short) color.getRed(),
						(short) color.getGreen(), (short) color.getBlue());
			}
		}
		return pImage;
	}
	...
	static BufferedImage pixImage2buffer(PixImage pImage) {
		BufferedImage bImage = new BufferedImage(pImage.getWidth(),
				pImage.getHeight(), BufferedImage.TYPE_INT_ARGB);
		for (int x = 0; x < bImage.getWidth(); x++) {
			for (int y = 0; y < bImage.getHeight(); y++) {
				bImage.setRGB(x, y,
						new Color(pImage.getRed(x, y), pImage.getGreen(x, y),
	 ...
	 * 
	 * @param rle
	 *            the input run-length encoded image.
	 * @param filename
	 *            the output filename.
	 */
	 ...
	 * 
	 * @param images
	 *            an array of PixImages to display.
	 */
	 ...
	 * 
	 * @param image
	 *            the PixImage to display.
	 */
	public static void displayTIFF(PixImage image) {
		displayTIFFs(new PixImage[] { image });
	}

	static int makeRGB(short r, short g, short b) {
		return (((int) r << 16) | ((int) g << 8) | b);
	}

	static short getRedValue(int pixel) {
		return (short) ((pixel & 0x00FF0000) >> 16);
	}

	static short getGreenValue(int pixel) {
		return (short) ((pixel & 0x0000FF00) >> 8);
	}

	static short getBlueValue(int pixel) {
		return (short) (pixel & 0x000000FF);
	}
}
