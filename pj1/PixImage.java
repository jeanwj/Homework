...
public class PixImage {
	...
	private static final int[][] gradientX = { { 1, 0, -1 }, { 2, 0, -2 }, { 1, 0, -1 } };
	private static final int[][] gradientY = { { 1, 2, 1 }, { 0, 0, 0 }, { -1, -2, -1 } };
	private static final int gradientSize = 3;
	...
	public short getRed(int x, int y) {
		// Replace the following line with your solution.
		return ImageUtils.getRedValue(this.pixels[x][y]);
	}
	...
	public short getGreen(int x, int y) {
		// Replace the following line with your solution.
		return ImageUtils.getGreenValue(this.pixels[x][y]);
	}
	...
	public short getBlue(int x, int y) {
		// Replace the following line with your solution.
		return ImageUtils.getBlueValue(this.pixels[x][y]);
	}
	...
	public void setPixel(int x, int y, short red, short green, short blue) {
		...
		this.pixels[x][y] = ImageUtils.makeRGB(red, green, blue);
	}
	...
	public String toString() {
		...
		for(int i=0;i<width;i++){
			...
			for(int j=0;j<height;j++){
				builder.append(this.pixels[i][j]);
				builder.append(',');
			}
	...
	public PixImage boxBlur(int numIterations) {
		...
		while (numIterations-- > 0) {
			PixImage newImage = new PixImage(image.width, image.height);
				...
				for (int y = 0; y < image.height; y++) {
					...
					for (int i = x - 1; i <= x + 1; i++) {
						...
						for (int j = y - 1; j <= y + 1; j++) {
							if (j < 0 || j > height - 1)
								continue;
							redSum += image.getRed(i, j);
							greenSum += image.getGreen(i, j);
							blueSum += image.getBlue(i, j);
						}
					}
					int size = image.boxSize(x, y);
					red = (short) (redSum / size);
					green = (short) (greenSum / size);
					blue = (short) (blueSum / size);
	...
	private int boxSize(int x, int y) {
		if (x == 0 || x == width - 1) {
			if (y == 0 || y == height - 1)
				return 4;
			else
			...
			for (int y = 0; y < this.height; y++) {
				...
				for (int i = 0; i < gradientSize; i++) {
					for (int j = 0; j < gradientSize; j++) {
						int xx = this.between(x + i - 1, 0, this.width - 1);
						int yy = this.between(y + j - 1, 0, this.height - 1);
						
						red = this.getRed(xx, yy);
		                green = this.getGreen(xx, yy);
		                blue = this.getBlue(xx, yy);

		                gxRed += (red * gradientX[i][j]);
		                gyRed += (red * gradientY[i][j]);
		                gxGreen += (green * gradientX[i][j]);
		                gyGreen += (green * gradientY[i][j]);
		                gxBlue += (blue * gradientX[i][j]);
		                gyBlue += (blue * gradientY[i][j]);
					}
				}
				
				long energy = (long)(Math.pow(gxRed, 2) + Math.pow(gyRed, 2) + 
						Math.pow(gxGreen, 2) + Math.pow(gyGreen, 2) + 
						Math.pow(gxBlue, 2) + Math.pow(gyBlue, 2));
				short pixel = mag2gray(energy);
                newImage.setPixel(x, y, pixel, pixel, pixel);
			}
		}
