...
public class RunIterator implements Iterator {
	 ...
	 */
	private DoubleLinkedListNode<Run> current;
	
	/**
	 * RunIterator() constructs a new iterator starting with a specified run.
	 * 
	 * @param node
	 *            the run where this iterator starts.
	 */
	// Unlike all the other methods we have asked you to write, the
	// RunIterator()
	...
	// of
	// the encoding.
	RunIterator(DoubleLinkedListNode<Run> node) {
	 ...
	 * sequence. It also advances the iterator to the next run, so that the next
	 * call to next() will return the following run.
	 * 
	 ...
	 *         The pixel count is in index [0]; the red value is in index [1];
	 *         the green value is in index [2]; and the blue value is in index
	 *         [3].
	 * @throws NoSuchElementException
	 *             if the iteration has no more elements. (We strongly recommend
	 *             calling hasNext() to check whether there are any more runs
	 *             before calling next().)
	 * 
	 *             The returned four-int array is constructed in next(), and can
	 *             be discarded by the calling method after use. The array
	 *             should not be part of your RunLengthEncoding data structure!
	 *             It must be freshly constructed for the sole purpose of
	 *             returning four ints.
	 */
	public int[] next() {
		...
		// Replace the following line with your solution.
		int[] data = new int[4];
		data[0] = this.current.value.count;
		data[1] = ImageUtils.getRedValue(current.value.pixel);
		data[2] = ImageUtils.getGreenValue(current.value.pixel);
		data[3] = ImageUtils.getBlueValue(current.value.pixel);
