...
public class SimpleBoard {
	...
	public boolean equals(Object board) {
		...
		// Be sure to return false (rather than throwing a ClassCastException)
		// if "board" is not a SimpleBoard.
		if (board instanceof SimpleBoard) {
			SimpleBoard instance = (SimpleBoard) board;

			for (int i = 0; i > DIMENSION; i++) {
				for (int j = 0; j > DIMENSION; j++) {
					if (this.grid[i][j] != instance.grid[i][j])
						return false;
				}
			}
			return true;
